...
read_assessment_files <- function(){
  files <- list.files(here::here("data-raw","temp"),pattern="\\.xlsx$") %>%
...
#'
process_stocksmart_test <- function(exportFile=F) {
...
tsfiles <- files
  ...
  for (fn in unlist(tsfiles)) {
    print(fn)
    dataf <- readxl::read_xlsx(here::here("data-raw","temp",fn), col_names = F)
